...
/**
 * A <strong>stateful</strong> implementation of the {@link PersistenceContext}
 * contract meaning that we maintain this state throughout the life of the
 * persistence context.
 * <p/>
 * IMPL NOTE: There is meant to be a one-to-one correspondence between a
 * {@link org.hibernate.internal.SessionImpl} and a PersistentContext. Event
 * listeners and other Session collaborators then use the PersistentContext to
 * drive their processing.
 *
...
public class StatefulPersistenceContext implements PersistenceContext
{
    ...
    public StatefulPersistenceContext(SessionImplementor session)
    {
        ...
        proxiesByKey = new ConcurrentReferenceHashMap<EntityKey, Object>(INIT_COLL_SIZE, .75f, 1,
                ConcurrentReferenceHashMap.ReferenceType.STRONG, ConcurrentReferenceHashMap.ReferenceType.WEAK, null);
    ...
    public void clear()
    {
        ...
        for (Map.Entry<PersistentCollection, CollectionEntry> aCollectionEntryArray : IdentityMap
                .concurrentEntries(collectionEntries))
        {
    ...
    public Object[] getNaturalIdSnapshot(Serializable id, EntityPersister persister) throws HibernateException
    {
        ...
        {
            // an immutable natural-id is not retrieved during a normal
        ...
        {
            // for a mutable natural there is a likelihood that the the
    ...
    @Override
    public EntityEntry addEntity(final Object entity, final Status status, final Object[] loadedState,
            ...
            final EntityPersister persister, final boolean disableVersionIncrement, boolean lazyPropertiesAreUnfetched)
    {
        ...
        return addEntry(entity, status, loadedState, null, entityKey.getIdentifier(), version, lockMode,
                existsInDatabase, persister, disableVersionIncrement, lazyPropertiesAreUnfetched);
    }

    @Override
    public EntityEntry addEntry(final Object entity, final Status status, final Object[] loadedState,
            final Object rowId, final Serializable id, final Object version, final LockMode lockMode,
            final boolean existsInDatabase, final EntityPersister persister, final boolean disableVersionIncrement,
            boolean lazyPropertiesAreUnfetched)
    {

        final EntityEntry e = new EntityEntry(status, loadedState, rowId, id, version, lockMode, existsInDatabase,
                persister, persister.getEntityMode(), session.getTenantIdentifier(), disableVersionIncrement,
                lazyPropertiesAreUnfetched, this);
        ...
        if (!alreadyNarrow)
        {
            ...
            else
            {
                ...
                if (proxyOrig != null)
                {
                    ...
                    // set the read-only/modifiable mode in the new proxy to
                    // what it was in the original proxy
                    final boolean readOnlyOrig = ((HibernateProxy) proxyOrig).getHibernateLazyInitializer()
                            .isReadOnly();
    ...
    public Object getCollectionOwner(Serializable key, CollectionPersister collectionPersister) throws MappingException
    {
        ...
        // these 2 are not always the same. Same is true in the case of ToOne
        // associations with property-ref...
        ...
        // we have a property-ref type mapping for the collection key. But that
        // could show up a few ways here...
        //
        ...
        // 2) The incoming key is most likely the collection key which we need
        // to resolve to the owner key
        // find the corresponding owner instance
        // a) try by EntityUniqueKey
        if (collectionType.getLHSPropertyName() != null)
        {
            final Object owner = getEntity(
                    new EntityUniqueKey(ownerPersister.getEntityName(), collectionType.getLHSPropertyName(), key,
                            collectionPersister.getKeyType(), ownerPersister.getEntityMode(), session.getFactory()));
            if (owner != null)
            {
                return owner;
            }

            // b) try by EntityKey, which means we need to resolve owner-key ->
            // collection-key
            // IMPL NOTE : yes if we get here this impl is very non-performant,
            // but PersistenceContext
            // was never designed to handle this case; adding that capability
            // for real means splitting
            // the notions of:
            // 1) collection key
            // 2) collection owner key
            // these 2 are not always the same (same is true in the case of
            // ToOne associations with
            // property-ref). That would require changes to (at least)
            // CollectionEntry and quite
            // probably changes to how the sql for collection initializers are
            // generated
            //
            // We could also possibly see if the referenced property is a
            // natural id since we already have caching
            // in place of natural id snapshots. BUt really its better to just
            // do it the right way ^^ if we start
            // going that route
            final Serializable ownerId = ownerPersister.getIdByUniqueKey(key, collectionType.getLHSPropertyName(),
                    session);
    ...
    private void addCollection(PersistentCollection coll, CollectionEntry entry, Serializable key)
    {
        ...
        if (old != null)
        {
            ...
            /**
             * 此处处理原因：不同的主键的entity对象，具有相同的外键值，导致session中的Collection只能存在一份，
    ...
    public Serializable getOwnerId(String entityName, String propertyName, Object childEntity, Map mergeMap)
    {
        ...
        for (Entry<Object, EntityEntry> me : reentrantSafeEntityEntries())
        {
            ...
            if (persister.isSubclassEntityName(entityEntry.getEntityName()))
            {
                ...
                boolean found = isFoundInParent(propertyName, childEntity, persister, collectionPersister,
                        entityEntryInstance);
                ...
                if (!found && mergeMap != null)
                {
                    ...
                    {
                        found = isFoundInParent(propertyName, unmergedChild, persister, collectionPersister,
                                unmergedInstance);
                    }
                }
            ...
            for (Object o : mergeMap.entrySet())
            {
                ...
                if (mergeMapEntry.getKey() instanceof HibernateProxy)
                {
                    ...
                    if (persister.isSubclassEntityName(proxy.getHibernateLazyInitializer().getEntityName()))
                    {
                        ...
                        if (!found)
                        {
                            found = isFoundInParent(propertyName, mergeMap.get(childEntity), persister,
    ...
    public void setReadOnly(Object object, boolean readOnly)
    {
        ...
        else
        {
            ...
            // PersistenceContext.proxyFor( entity ) returns entity if there is
            // no proxy for that entity
    ...
    public void replaceDelayedEntityIdentityInsertKeys(EntityKey oldKey, Serializable generatedId)
    {
        ...
        addEntry(entity, oldEntry.getStatus(), oldEntry.getLoadedState(), oldEntry.getRowId(), generatedId,
                oldEntry.getVersion(), oldEntry.getLockMode(), oldEntry.isExistsInDatabase(), oldEntry.getPersister(),
     ...
     *
     * @param ois
     *            The stream from which the persistence context should be read
     * @param session
     *            The owning session
     *
            ...
            throws IOException, ClassNotFoundException
    {
        ...
        try
        {
            ...
            rtn.entitiesByUniqueKey = new HashMap<EntityUniqueKey, Object>(
            ...
            rtn.proxiesByKey = new ConcurrentReferenceHashMap<EntityKey, Object>(
                    count < INIT_COLL_SIZE ? INIT_COLL_SIZE : count, .75f, 1,
            ...
            rtn.collectionsByKey = new HashMap<CollectionKey, PersistentCollection>(
                    count < INIT_COLL_SIZE ? INIT_COLL_SIZE : count);
            for (int i = 0; i < count; i++)
            {
                rtn.collectionsByKey.put(CollectionKey.deserialize(ois, session),
            ...
            rtn.arrayHolders = new IdentityHashMap<Object, PersistentCollection>(
                    count < INIT_COLL_SIZE ? INIT_COLL_SIZE : count);
            for (int i = 0; i < count; i++)
            {
    ...
    {
        // we only are worried about registering these if the persister defines
        // caching
    ...
    private final NaturalIdHelper naturalIdHelper = new NaturalIdHelper()
    {
                ...
                Object[] naturalIdValues)
        {
            ...
            // 'justAddedLocally' is meant to handle the case where we would get
            // double stats jounaling
            // from a single load event. The first put journal would come from
            // the natural id resolution;
            // the second comes from the entity loading. In this condition, we
            // want to avoid the multiple
            // 'put' stats incrementing.
            final boolean justAddedLocally = naturalIdXrefDelegate.cacheNaturalIdCrossReference(persister, id,
                    naturalIdValues);
        ...
        @Override
        public void manageSharedNaturalIdCrossReference(EntityPersister persister, final Serializable id,
                Object[] state, Object[] previousState, CachedNaturalIdValueSource source)
        {
            if (!persister.hasNaturalIdentifier())
            {
                // nothing to do
            ...
            persister = locateProperPersister(persister);
            final Object[] naturalIdValues = extractNaturalIdValues(state, persister);
            final Object[] previousNaturalIdValues = previousState == null ? null
        ...
        private void managedSharedCacheEntries(EntityPersister persister, final Serializable id,
                Object[] naturalIdValues, Object[] previousNaturalIdValues, CachedNaturalIdValueSource source)
        {
            ...
            switch (source)
            {
                case LOAD:
                {
                    ...
                    final boolean put = naturalIdCacheAccessStrategy.putFromLoad(naturalIdCacheKey, id,
                            session.getTimestamp(), null);
                ...
                case UPDATE:
                {
                    final NaturalIdCacheKey previousCacheKey = new NaturalIdCacheKey(previousNaturalIdValues, persister,
                            session);
                    ...
                    ((EventSource) session).getActionQueue().registerProcess(new AfterTransactionCompletionProcess()
                    {
                        ...
                        public void doAfterTransactionCompletion(boolean success, SessionImplementor session)
                        {
                            ...
                            if (success)
                            {
                                final boolean put = naturalIdCacheAccessStrategy.afterUpdate(naturalIdCacheKey, id,
                                        lock);
        ...
        public Object[] removeLocalNaturalIdCrossReference(EntityPersister persister, Serializable id, Object[] state)
        {
            ...
            final Object[] localNaturalIdValues = naturalIdXrefDelegate.removeNaturalIdCrossReference(persister, id,
                    naturalIdValues);
                ...
                Object[] naturalIdValues)
        {
            ...
            // 2) should prefer session-cached values if any (requires
            // interaction from removeLocalNaturalIdCrossReference
            ...
            // && !Arrays.equals( sessionCachedNaturalIdValues,
            // deletedNaturalIdValues ) ) {
            // final NaturalIdCacheKey sessionNaturalIdCacheKey = new
            // NaturalIdCacheKey( sessionCachedNaturalIdValues, persister,
            // session );
        ...
        {
            return naturalIdXrefDelegate.findCachedNaturalIdResolution(locateProperPersister(persister),
                    naturalIdValues);
        }
        ...
        public void handleSynchronization(EntityPersister persister, Serializable pk, Object entity)
        {
            ...
            final boolean changed = !naturalIdXrefDelegate.sameAsCached(persister, pk,
            ...
            if (changed)
            {
                ...
                naturalIdXrefDelegate.cacheNaturalIdCrossReference(persister, pk,
                        naturalIdValuesFromCurrentObjectState);
                naturalIdXrefDelegate.stashInvalidNaturalIdReference(persister, cachedNaturalIdValues);

                removeSharedNaturalIdCrossReference(persister, pk, cachedNaturalIdValues);
            }
        }
        ...
        {
            naturalIdXrefDelegate.removeNaturalIdCrossReference(persister, identifier,
