...
public final class SessionFactoryImpl implements SessionFactoryImplementor {
	...
	private MutableIdentifierGeneratorFactory generatorFactory = null;
	...
	public SessionFactoryImpl(final Configuration cfg, Mapping mapping, final ServiceRegistry serviceRegistry,
			Settings settings, SessionFactoryObserver observer) throws HibernateException {
		...
		this.serviceRegistry = serviceRegistry.getService(SessionFactoryServiceRegistryFactory.class)
		...
		// Generators:
		identifierGenerators = new ConcurrentHashMap();
		
		generatorFactory = cfg.getIdentifierGeneratorFactory();
		Iterator classes = cfg.getClassMappings();
		while (classes.hasNext()) {
			PersistentClass model = (PersistentClass) classes.next();
			registerIdGenerator(model);
		}
		...
		regionFactory = cacheAccess.getRegionFactory();
		cacheRegionPrefix = settings.getCacheRegionPrefix() == null ? "" : settings.getCacheRegionPrefix() + ".";
		persisterFactory = serviceRegistry.getService(PersisterFactory.class);

		// todo : consider removing this silliness and just have EntityPersister
		// directly implement ClassMetadata
		// EntityPersister.getClassMetadata() for the internal impls simply
		// "return this";
		// collapsing those would allow us to remove this "extra" Map
		//
		// todo : similar for CollectionPersister/CollectionMetadata

		entityPersisters = new ConcurrentHashMap();
		classMetadata = new ConcurrentHashMap<String, ClassMetadata>();
		entityAccessStrategies = new ConcurrentHashMap();

		classes = cfg.getClassMappings();
		while (classes.hasNext()) {
			registerPersisterAndClassMeta(mapping, (PersistentClass) classes.next());
		}

		collectionRolesByEntityParticipant = new ConcurrentHashMap<String, Set<String>>();
		collectionPersisters = new ConcurrentHashMap<String, CollectionPersister>();
		collectionMetadata = new ConcurrentHashMap<String, CollectionMetadata>();
		
		Iterator collections = cfg.getCollectionMappings();
		while (collections.hasNext()) {
			registerCollection(cfg, (Collection) collections.next());
		}

		// Named Queries:
		this.namedQueryRepository = new NamedQueryRepository(cfg.getNamedQueries().values(),
				cfg.getNamedSQLQueries().values(), cfg.getSqlResultSetMappings().values(),
		...
		SessionFactoryRegistry.INSTANCE.addSessionFactory(uuid, name, settings.isSessionFactoryNameAlsoJndiName(), this,
				serviceRegistry.getService(JndiService.class));

		LOG.debug("Instantiated session factory");

		settings.getMultiTableBulkIdStrategy().prepare(jdbcServices, buildLocalConnectionAccess(), cfg.createMappings(),
	...
	private void registerCollection(Collection model) {
		final String cacheRegionName = cacheRegionPrefix + model.getCacheRegionName();
		final AccessType accessType = AccessType.fromExternalName(model.getCacheConcurrencyStrategy());
		CollectionRegionAccessStrategy accessStrategy = null;
		if (accessType != null && settings.isSecondLevelCacheEnabled()) {
			LOG.tracev("Building shared cache region for collection data [{0}]", model.getRole());
			CollectionRegion collectionRegion = regionFactory.buildCollectionRegion(cacheRegionName, properties,
					CacheDataDescriptionImpl.decode(model));
			accessStrategy = collectionRegion.buildAccessStrategy(accessType);
			entityAccessStrategies.put(cacheRegionName, accessStrategy);
			cacheAccess.addCacheRegion(cacheRegionName, collectionRegion);
		}
		CollectionPersister persister = persisterFactory.createCollectionPersister(cfg, model, accessStrategy,
				this);
		collectionPersisters.put(model.getRole(), persister);
		collectionMetadata.put(model.getRole(), persister.getCollectionMetadata());
	...
	/**
	 * For dynamic register
	 */
	private void registerPersisterAndClassMeta(Mapping mapping, final PersistentClass model) {
			...
			if (naturalIdAccessStrategy == null && settings.isSecondLevelCacheEnabled()) {
				...
				try {
					...
					LOG.warnf(
							...
							regionFactory.getClass().getName(), model.getEntityName());
				}
		...
		EntityPersister cp = persisterFactory.createEntityPersister(model, accessStrategy, naturalIdAccessStrategy,
				this, mapping);
	 ...
	 * For dynamic register
	 */
	private void registerIdGenerator(PersistentClass model) {
		if (!model.isInherited()) {
			IdentifierGenerator generator = model.getIdentifier().createIdentifierGenerator(generatorFactory,
	...
	private CustomEntityDirtinessStrategy determineCustomEntityDirtinessStrategy() {
		CustomEntityDirtinessStrategy defaultValue = new CustomEntityDirtinessStrategy() {
			...
			@Override
			public void findDirty(Object entity, EntityPersister persister, Session session,
		...
		return serviceRegistry.getService(ConfigurationService.class).getSetting(
				AvailableSettings.CUSTOM_ENTITY_DIRTINESS_STRATEGY, CustomEntityDirtinessStrategy.class, defaultValue);
	}
	...
	private CurrentTenantIdentifierResolver determineCurrentTenantIdentifierResolver(
		...
		return serviceRegistry.getService(ConfigurationService.class).getSetting(
				AvailableSettings.MULTI_TENANT_IDENTIFIER_RESOLVER, CurrentTenantIdentifierResolver.class, null);
	...
	public Session openTemporarySession() throws HibernateException {
		return withOptions().autoClose(false).flushBeforeCompletion(false)
				.connectionReleaseMode(ConnectionReleaseMode.AFTER_STATEMENT).openSession();
	}
				...
				if (isMappedClass) {
					...
					if (mappedClass != null && clazz.isAssignableFrom(mappedClass)) {
						...
						if (checkQueryable.isInherited()) {
							Class mappedSuperclass = getEntityPersister(checkQueryable.getMappedSuperclass())
									.getMappedClass();
	...
	public void close() throws HibernateException {
		...
		SessionFactoryRegistry.INSTANCE.removeSessionFactory(uuid, name, settings.isSessionFactoryNameAlsoJndiName(),
				serviceRegistry.getService(JndiService.class));
	...
	private CurrentSessionContext buildCurrentSessionContext() {
		...
		if ("jta".equals(impl)) {
			...
			try {
				...
				return (CurrentSessionContext) implClass.getConstructor(new Class[] { SessionFactoryImplementor.class })
						.newInstance(this);
			} catch (Throwable t) {
	...
	static class SessionBuilderImpl implements SessionBuilderImplementor {
		...
		public Session openSession() {
			...
			final SessionImpl session = new SessionImpl(connection, sessionFactory, sessionOwner,
					getTransactionCoordinator(), autoJoinTransactions,
					sessionFactory.settings.getRegionFactory().nextTimestamp(), interceptor, flushBeforeCompletion,
					autoClose, connectionReleaseMode, tenantIdentifier);
